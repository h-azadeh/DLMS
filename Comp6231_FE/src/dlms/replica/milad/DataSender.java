...
public class DataSender implements Runnable{
	...
	public DataSender(RemoteBankImpl aBankImpl, int aReceivingPort, String anotherReplica1, String anotherReplica2){
		   ...
		   while(true){
			    ...
			    while(true){
					if(myBankImpl.isProcessing.equals(1))
					{
					    ...
					    Hashmaper completeHashMaps = new Hashmaper(myBankImpl.AccountDB, myBankImpl.LoanDB, randomNumber);
			    		ByteArrayOutputStream bStream  = new ByteArrayOutputStream();
			    		ObjectOutputStream out = new ObjectOutputStream(bStream);
			    		out.writeObject(completeHashMaps);
			    		out.flush();
			    		DatagramPacket reply = new DatagramPacket(bStream.toByteArray(),
			    		    		bStream.size(), request.getAddress(), Integer.parseInt(receivedReq));
			    		aSocket.send(reply);	    		
			    		break;
			    		}
			    ...
			    if(receivedReq.equals("done"))
			    {
			    	InetAddress myBuddy = InetAddress.getByName(myBankImpl.myBuddyIP);
