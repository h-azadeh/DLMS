...
public class Replica implements UDPNotifierIF
{
	...
	public Replica()
	{
		...
		{
			addServer(ReplicaConfiguration.PORT_POOL[i],
					ReplicaConfiguration.BANK_NAME_POOL[i]);
		}

		// create multicast receiver
		multicastReceiver = new MulticastReceiver(this,
				Configuration.MULTI_CAST_GROUP_PORTS[0]);
	...
	private void addServer(int udpPort, String string)
	{
		...
		DataUpdater updater = new DataUpdater(server,
				ReplicaConfiguration.UPDATER_PORT_POOL[counter],
				9167+ counter,
				Integer.toString(9281+counter),
				9360+counter,
				ReplicaConfiguration.UDP_FE_ANOTHER_IP_1,
				ReplicaConfiguration.UDP_FE_ANOTHER_IP_2);
	...
	public String openAccount(String firstName, String lastName,
			String emailAddress, String phoneNumber, String password,
			int bankId)
	{
		// return empty string if the bank name can't be found in the server
		// list
		return getServerById(bankId) == null ? ""
				: getServerById(bankId).openAccount(
	...
	public boolean getLoan(String accountNumber, String password,
			double loanAmount, int bankId)
	{
		// return empty string if the bank name can't be found in the server
		// list
		return getServerById(bankId) == null ? false
				: !getServerById(bankId).getLoan(
						ReplicaConfiguration.BANK_NAME_POOL[bankId - 1],
						accountNumber, password, loanAmount).equals("");
	}

	public boolean delayPayment(String loanID, String currentDueDate,
			String newDueDate, int bankId)
	{
		// return false if the bank name can't be found in the server list
		return getServerById(bankId) == null ? false
				: getServerById(bankId).delayPayment(
						ReplicaConfiguration.BANK_NAME_POOL[bankId - 1], loanID,
	...
	public String[] printCustomerInfo(int bankId)
	{
		// return empty string if the bank name can't be found in the server
		// list
		return getServerById(bankId) == null ? new String[]
		{}
				: getServerById(bankId).printCustomerInfoToArray(
						ReplicaConfiguration.BANK_NAME_POOL[bankId - 1]);
	}

	public boolean transferLoan(String LoanID, String CurrentBank,
			String OtherBank, int bankId)
	{
		// return empty string if the bank name can't be found in the server
		// list
		return getServerByName(CurrentBank) == null ? false
				: !getServerByName(CurrentBank)
	...
	public void notifyMessage(UDPProtocol message)
	{
		...
		case OPEN_ACCOUNT:
			...
			ret = openAccount(message.getClientRequest().getFirstName(),
					message.getClientRequest().getLastNmae(),
					message.getClientRequest().getEmail(),
					message.getClientRequest().getPhoneNum(),
			...
			try
			{
				...
				UDPSender.sendUDPPacket(fe.main.Configuration.FE_IP,
						fe.main.Configuration.FE_PORT, message);
			} catch (IOException e)
			{
				e.printStackTrace();
			}
		...
		case GET_LOAN:
			...
			ret = getLoan(
					Integer.toString(message.getClientRequest().getAccountId()),
					message.getClientRequest().getPassWord(),
		...
		case DELAY_LOAN:
			...
			ret = delayPayment(
					Integer.toString(message.getClientRequest().getLoanId()),
					message.getClientRequest().getCurrentDueDate(),
					message.getClientRequest().getNewDueDate(),
					message.getFeHeader().getBankId());
			if (SoftwareBugSimulator.shouldGenerateWrongMessage())
			{
				ret = false;
		...
		case TRANSFER_LOAN:
			...
			ret = transferLoan(
					Integer.toString(message.getClientRequest().getLoanId()),
					message.getClientRequest().getCurrentBank(),
					message.getClientRequest().getOtherBank(),
					message.getFeHeader().getBankId());
			if (SoftwareBugSimulator.shouldGenerateWrongMessage())
			{
				ret = false;
